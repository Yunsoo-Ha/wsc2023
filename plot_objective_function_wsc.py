...
for problem_name in problems:
    ...
    file_name = f"{'ASTRODFRP_problem'}_on_{problem_name}"
    experiment = read_experiment_results(f"experiments/outputs/{file_name}.pickle")
    experiment.solver.name = "History-informed ASTRO-DF"
    myexperiment.append(experiment)

    file_name = f"{'ASTRODFRF_problem'}_on_{problem_name}"
    experiment = read_experiment_results(f"experiments/outputs/{file_name}.pickle")
    experiment.solver.name = "ASTRO-DF (interpolation)"
    myexperiment.append(experiment)

    file_name = f"{'ASTRODFREG'}_on_{problem_name}"
    experiment = read_experiment_results(f"experiments/outputs/{file_name}.pickle")
    experiment.solver.name = "ASTRO-DF (regression)"
    myexperiment.append(experiment)
